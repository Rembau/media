...
public class Addapplication extends ServletFather {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(rs.getString(5).equals("占用")){ 
			markstate=1;
			out.print("<script>alert('该时段被占用');window.location.href='addapplication.jsp';</script>");
			return;
		}
		}
		if(alljie.equals("3"))
		...
		while(rs.next()){
			markstate=2;
			 out.print("<script>alert('"+username+"该时段已经有课');window.location.href='addapplication.jsp';</script>");
			 return;
		}
		...
		{
			markstate=3;
			out.print("<script>alert('课时不足');window.location.href='addapplication.jsp?area="+area+"&week="+week+"&weekday="+weekday+"&jieci="+jieci+"&ok=0';</script>");
    		new log.Logs().getloga().info(session.getAttribute("num")+" "+username+"课时不足"+sql);
			out.flush();
			out.close();
		}
