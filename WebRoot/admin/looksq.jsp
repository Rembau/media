...
String weekday=null;
	 ...
	 else {weekday=intweekday.toString();}
	 //out.print(intweekday.toString());
int intjieci=-1;	 
String jieci=null;
...
String strintjieci=null;
     ...
     System.out.print("strintjieci"+strintjieci);
	 System.out.println("intjieci"+intjieci);
...
function changes(id,mark){
   var chose=document.getElementById(id).value;
   window.location.href='.ChangeS?mark='+mark+'&id='+id+'&chose='+chose+'&area='+area+'&intjieci='+intjieci+'&week='+week+'&weekday='+weekday+'&floor1='+floor1+'&layer='+layer+'&room='+room+'&page=<%=nowpage%>';
}
  function changeweek(x)
	...
	window.location.href='looksq.jsp?area='+area+'&intjieci='+intjieci+'&week='+week+'&weekday='+weekday+'&floor1='+floor1+'&layer='+layer+'&room='+room+'&name1=<%=name1%>&num1=<%=num1%>';
...
sql="select * from media_application where ap_state='通过'";
//out.print(sql+" "+strnum+" "+strweekday+" "+strarea+" "+strjieci+" "+strroom1+" "+strname+" "+strweek);
rs=conn.stmt.executeQuery(sql+" "+strweek+" "+strweekday+" "+strarea+" "+strjieci+" "+strroom1+" "+strname+" "+strnum+" order by ap_room asc");
...
<%=introom%1000%></td>
	 ...
	 <td>
	 <a href="?page=<%=nowpage%>&week=<%=week%>&weekday=<%=weekday%>&intjieci=<%=intjieci%>&area=<%=area%>&room=<%=room%>&floor1=<%=floor1%>&layer=<%=layer%>&teacher=<%=rs.getString(10)%>"><%=rs.getString(2)%></a>
	 </td>
	 <td><%=rs.getString(10)%></td><td><%=rs.getString(8)%></td><td><%=rs.getString(11)%></td>
	 <td>
	 <select id=<%=rs.getString("ap_id")%> onchange="changes(this.id,'ap_normal')">
	 <option <%if(rs.getString("ap_normal").equals("是")) out.print("selected");%> value="是">是</option>
	 <option <%if(rs.getString("ap_normal").equals("否")) out.print("selected");%> value="否">否</option>
	 <option <%if(rs.getString("ap_normal").equals("未知")) out.print("selected");%> value="未知">未知</option>
	 </select>
	 </td>
	 <td>
	 <select id=<%=0+rs.getString("ap_id")%> onchange="changes(this.id,'ap_used')">
	 <option <%if(rs.getString("ap_used").equals("是")) out.print("selected");%> value="是">是</option>
	 <option <%if(rs.getString("ap_used").equals("否")) out.print("selected");%> value="否">否</option>
	 <option <%if(rs.getString("ap_used").equals("未知")) out.print("selected");%> value="未知">未知</option>
	 </select>
	 </td>
	 </tr>	
	 <%k++;
	...
	<td><%if(nowpage>1){%>
	<a href="?page=<%=nowpage-1%>&week=<%=week%>&weekday=<%=weekday%>&intjieci=<%=intjieci%>&area=<%=area%>&room=<%=room%>&floor1=<%=floor1%>&layer=<%=layer%>&endweek=<%=endweek%>&name1=<%=name1 %>&num1=<%=num1 %>">上一页</a><%}%></td>
