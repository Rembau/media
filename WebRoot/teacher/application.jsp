...
String week=null;
	 ...
	 //out.print(week+""+intweekday.toString()+""+inthour.toString());
...
String floor1=null;
     ...
     ResultSet rs3= conn.getOldStmt().executeQuery("select * from media_site where sit_area = '"+area+"'");
     if(rs3.next()){
          floor1=rs3.getString(3);
       }
	 }
...
String layer=null;
	 ...
	 //out.print(area+" "+jieci+" "+week+" "+weekday);
	  String strjieci[]={"第一节","第二节","第三节","第四节","第五节","第六节","第七节","第八节","第九节","第十节","第十一节","第十二节"};
   %>
<span class="current"><font color="#F00">当前位置:</font>教师申请(必须在上课前一天晚上九点前完成选课)</span>
...
<br>
课程:&nbsp;&nbsp;
课程名<select name="courseName" id="courseName">
<%sql="select * from media_course where co_num='"+num.toString()+"'"; 
rs=stmt.executeQuery(sql);
while(rs.next()){
 sql="select * from sqcount where ap_num='"+num+"' and ap_courseName='"+rs.getString("co_courseName")+"'";
 ResultSet rs2 = conn.getStmt().executeQuery(sql);
 int count=0;
 if(rs2.next())
 {
 	count=rs2.getInt("courseNum");
 }
  ...
  function changeweek(x)
	...
	{alert("节次输入错误");
     window.location.reload();
      ...
      window.location.href='application.jsp?area='+area+'&jieci='+jieci+'&week='+week+'&weekday='+weekday+'&alljie='+alljie+'&floor1='+floor1+'&layer='+layer+'';
...
if(alljie.equals("2"))
{ 
	sql="select * from media_situation where si_area='"+area+"'  and si_week='"+week+"'and si_weekday='"+weekday+"'and si_jieci in('"+strjieci[jiecistart]+"','"+strjieci[jiecistart+1]+"') "+strroom1 +"order by si_room asc"; 
}// si_state='正常' and and si_state='占用'
else  if(alljie.equals("3")) 
{
	sql="select * from media_situation where si_area='"+area+"' and si_week='"+week+"'and si_weekday='"+weekday+"'and si_jieci in('"+strjieci[jiecistart]+"','"+strjieci[jiecistart+1]+"','"+strjieci[jiecistart+2]+"') "+strroom1 +"order by si_room asc";
}
 //out.print(sql);
   rs=stmt.executeQuery(sql);
...
while(rows>0){  out.print("<tr>"); 
 cols=remember;
  ...
  //out.print(sql+" "+introom);
  sql="select * from media_site where sit_nummark='"+introom/1000+"'";
//out.print(sql);
ResultSet rs2=stmt1.executeQuery(sql);
rs2.next();
floor2=rs2.getString(2);
  out.print("<td heigh='20'>");
  int intmark=0;
  if(rs.getString(10).equals("不能选课")){
  intmark=1;
  }
  out.print("<a href='?week="+week+"&area="+area+"&weekday="+weekday+"&jieci="+strjieci[jiecistart]+"&ok="+rs.getString(3)+"&alljie="+alljie+"&floor1="+floor1+"&layer="+layer+"' onClick='return app("+introom+","+intmark+")'>申请&nbsp;"+area+""+floor2+""+introom%1000+"</a><br>目前状态:");
  if(!rs.getString(10).equals("不能选课")){
  if(rs.getString(4).equals("故障")){
...
out.print("<font color=red>");
  ...
  out.print("<br>"+rs.getString(8)+":");
  if(rs.getString(5).equals("占用")){out.print("<font color=red>"+rs.getString(5)+"</font>");} 
  else out.print(rs.getString(5));
...
<form action='teacher.Applicationruku?week=<%=week%>&area=<%=area%>&weekday=<%=weekday%>&jieci=<%=jieci%>&room=<%=ok%>&alljie=<%=alljie%>' method="post" name="app">
...
<%}%>
<script type="text/javascript">
function app(room,state){
var course=document.form.courseName.value;
if(course=='')
{
	alert('您没有任何课时可选！');
	return false;
}
var s="你选择了:\n<%=area%><%=floor2%>"+room%1000+",第<%=week%>周,<%=weekday%>,<%=jieci%>,时间段\n课程名："+course;
if(state==1){
  alert('节假日期间不能选课');
  return false;
}
if(window.confirm(s)){
   window.location.href='teacher.Applicationruku?week=<%=week%>&area=<%=area%>&weekday=<%=weekday%>&jieci=<%=jieci%>&room='+room+'&alljie=<%=alljie%>&courseName='+course;
}
return false;
}
</script>
