...
<%String  sql;
ResultSet rs;
sql="select * from media_fault where fa_state='已确认' order by fa_time desc";
 rs=stmt.executeQuery(sql);%>
<span class="current"><font color="#F00">当前位置:</font>故障教室</span><br>
...
%><%=floor2%>
<%=introom%1000%><%}%></td><td><%=rs.getString(6)%></td><td><%=rs.getString(7)%></td><td><%=rs.getString(9)%></td><td><%=rs.getString(14)%></td>
	 <td><a href="fault.jsp?id=<%=rs.getString(1)%>&page=<%=nowpage%>&markpage=2">点击查看详细内容</a></td> </tr>
